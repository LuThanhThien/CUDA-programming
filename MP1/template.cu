...
__global__ void vecAdd(float *in1, float *in2, float *out, int len) {
  ...
  int index = blockIdx.x * blockDim.x + threadIdx.x; //0
...
int main(int argc, char **argv) {
  ...
  int numThreads = 256;
  dim3 dimThreads(numThreads, 1, 1);  //1D
  dim3 dimBlock((inputLength + numThreads + 1) / numThreads, 1, 1); //1D

  wbTime_start("Compute", "Performing CUDA computation");
  //@@ Launch the GPU Kernel here
  vecAdd<<<dimBlock, dimThreads>>>(deviceInput1, deviceInput2, deviceOutput, inputLength);
